...
def login_user(request):
    ...
    try:
        ...
        password = data.get("password", "")
        login_type = data.get("loginType", "user").lower()  # 'admin' or 'user'
        ...
        if user is not None:
            ...
            # AUTO-DETECT: Check if user is admin
            is_admin = user.is_staff and user.is_superuser
            
            # Determine redirect based on user privileges
            if is_admin:
                ...
                user_type = "user"
                print(f"Redirecting to user dashboard")
            
            logger.info(f"User logged in successfully: {email} (type: {user_type})")
...
def admin_dashboard(request):
    """Admin dashboard - SECURED with @login_required + admin check"""
